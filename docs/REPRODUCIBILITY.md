...
| PPO | KL target | 0.02; the remaining epochs are skipped when the mean approximate KL of an epoch exceeds 1.5×0.02 |
...
- Source: HTT-RL ref. BC log is not in the repository; 88.2% is taken from `CLAUDE.md` (project notes) only -> to be confirmed by the authors

### Differences between the published runs and commit `1253eea`

HTT-RL<sub>ref.</sub> was trained with an earlier version of `scripts/train_hydra.py`, which sized the cosine schedule to 671 updates with 26 warmup updates. At `1253eea`, the launcher multiplies this estimate by 1.25, so the command of table *Commands of the pipeline* (Appendix G.2 of the paper) gives 839 and 33, the schedule of PO-HTT-RL; the override `tu_per_decision=30` restores 671 and 26. The commit that added this factor (`c8a758f`) also corrected the rotation of the training layouts: before it, each block of five episodes of a worker contained four episodes of one layout and one episode of the next. HTT-RL<sub>ref.</sub> was trained before this correction and PO-HTT-RL, the know. fine-tune, and the MLP anchors after it, so their sequences of training layouts are not identical. For PO-HTT-RL, the behavior-cloning data were re-collected with the same command and seed on the RTX 4080. Training is not bit-reproducible, because the learner process samples actions with the unseeded PyTorch generator.

- Source: `git show 255560a:scripts/train_hydra.py` (`est_rounds` without the 1.25 factor -> `int(600/5*(275000/24)/2048)=671`, warmup `max(10,671//25)=26`); `c8a758f` message and diff ("1.25 schedule inflation"; "scenario-rotation off-by-one fixed in BOTH training env paths")
...
- Source: `scripts/local_ft_gini_queue.sh` added in `ad07add` (after `c8a758f`); `train_po_v6` at `bd43ded` (after); trad queue added in `c8a758f`. Launch commit of the qua./fat./pro. fine-tunes is not recorded (queue `bc57d5f` predates `c8a758f` by 2 h).
...
- Source: evaluation layouts computed with `RandomScenarioSampler(KATAConfig(<config>), randomized_scenario, seed=20260722).sample_config()` at `1253eea`
