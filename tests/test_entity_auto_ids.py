"""Tests for auto-incrementing entity IDs."""
...
# The _reset_entity_id_counters fixture from conftest.py runs automatically.
...
def test_entity_ids_are_auto_incremented_per_class():
    ...
    router_1 = Router(
        env=env, name="router1", in_buffer=buffer_a, type_to_buffer={"x": buffer_b}
    )
    router_2 = Router(
        env=env, name="router2", in_buffer=buffer_a, type_to_buffer={"x": buffer_b}
    )
