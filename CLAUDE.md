...
- ~~Masked-heuristics re-run~~ DONE 2026-07-15 (all 4 scenarios) and §6 fully refreshed (tables + prose; spiral sentence removed). New headline: PO-RL never beats informed heuristics; HC-RL is the only agent that does (industrial rank 1.83); at 5M saturation, fatigue-aware rules win outright (SQ 104.8k). Co-author merge dfb4f08 integrated (their intro/abstract kept, HTT-RL stays contribution #2). Local commits f22ebff (paper) / 1d91dc0 (parent) **not yet pushed**.
