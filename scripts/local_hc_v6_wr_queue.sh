#!/usr/bin/env bash
# hc_v6_wr queue on the LOCAL machine — the CORRECTED warm-restart
# extension of hc_v6 (companion to hc_v6_ext).
#
# hc_v6_ext (2026-08-31) was meant to warm-restart the LR at 1e-4 but
# the re-arm gate missed the same-size-extension case (ckpt
# last_epoch 832 vs total_updates ~837), so it trained all 600 eps at
# the floor 1.5e-5 — a valid "more budget at tail LR" data point, but
# not the intended plasticity test.  With the gate fixed (re-arm on
# every restore), this queue runs the intended variant: same init
# (hc_v6 final.pt), same recipe/reward, seed 44 (fresh worlds,
# distinct from ext's 43), lr=1e-4 cosine actually applied.
#
# GATED on the ext queue finishing (shares the GPU).
set -u
cd "$(dirname "$0")/.."
export PYTHONPATH="$(pwd)/src${PYTHONPATH:+:$PYTHONPATH}"
export WANDB_MODE=offline
export OMP_NUM_THREADS=2
Q=reports/hc_v6_wr_queue.log
...
say() { echo "$(date -u +%FT%TZ) [hc-v6-wr] $*" | tee -a "$Q"; }
mkdir -p reports checkpoints

[ -f "$INIT" ] || { say "ABORT: init checkpoint $INIT missing"; exit 1; }
say "HC V6 WR QUEUE ARMED (pid $$, seed $SEED, lr 1e-4 WARM RESTART, +600 eps from hc_v6 final)"
until grep -q "HC V6 WR GATE OPEN\|HC V6 EXT QUEUE DONE" reports/hc_v6_ext_queue.log 2>/dev/null; do
  say "gate: waiting for HC V6 EXT QUEUE DONE (GPU busy) — 15 min"
  sleep 900
done
say "gate passed: ext queue done"
