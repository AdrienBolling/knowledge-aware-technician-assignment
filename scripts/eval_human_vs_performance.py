...
CHECKPOINTS = {
    ...
    "ft_gini": Path("checkpoints/ft_gini/set_transformer_best.pt"),
    "ft_gini_last": Path("checkpoints/ft_gini/set_transformer_last.pt"),
...
SCENARIOS = {
    ...
    # Neutral-turnover variants of lifecycle (reviewer point: S5 retires
    # the most knowledgeable technicians, a targeted key-person stress
    # test).  Generated by scripts/make_turnover_configs.py; everything
    # except the retirement selection (and, for random_timing, the
    # retire/hire pair times) is identical to lifecycle.
    "lifecycle_random_retire": dict(
        cfg="run_configs/benchmark_suite/lifecycle_random_retire.json",
        n_eps=1, sim=5_000_000.0, steps=1_500_000,
    ),
    "lifecycle_random_timing": dict(
        cfg="run_configs/benchmark_suite/lifecycle_random_timing.json",
        n_eps=1, sim=5_000_000.0, steps=1_500_000,
    ),
    ...
    for scenario in names:
        ...
        for label, (agent, env) in agents.items():
            for ep in range(n_eps):
                ...
                # Lifecycle targets (which technicians/machines left) —
                # policy-dependent under highest_knowledge, fixed per
                # seed under random selection.
                for ev in getattr(env, "_lifecycle_log", None) or []:
                    lc_rows.append({"agent": label, "episode": ep, **ev})
